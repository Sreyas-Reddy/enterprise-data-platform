...
CREATE OR REPLACE VIEW DQ_SCORECARD_V2 AS
...
SELECT
  ...
  CASE
    WHEN r.STATUS = 'FAILED' OR COALESCE(t.tests_failed, 0) > 0 OR COALESCE(e.exception_rows, 0) > 0
      THEN 'FAIL'
    ELSE 'PASS'
  ...
  CASE
    ...
    ELSE ROUND(100 * (COALESCE(t.tests_logged,0) - COALESCE(t.tests_failed,0)) / COALESCE(t.tests_logged,0), 2)
  END AS test_pass_percentage
FROM DQ_RUNS r
LEFT JOIN test_summary t ON r.RUN_ID = t.RUN_ID
LEFT JOIN exception_summary e ON r.RUN_ID = e.RUN_ID;
